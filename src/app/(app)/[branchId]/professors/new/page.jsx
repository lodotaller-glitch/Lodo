...
export default function NewProfessorPage({ params }) {
  ...
  const [loading, setLoading] = useState(false); // <-- controla overlay/spinner
  ...
  async function onSubmit(e) {
    ...
    // --- Validaciones rápidas ANTES de activar loading (evita parpadeo)
    ...
    setLoading(true); // <-- ahora sí, ocultamos el form y mostramos overlay
    ...
    try {
      ...
      setLoading(false); // si no hay redirect, volvemos a mostrar el form
    }
  }

  return (
    <main className="max-w-3xl mx-auto p-6 relative">
      {/* Overlay de carga que TAPA todo y oculta inputs/botones */}

      <h1 className="text-2xl font-semibold mb-4">
        Crear Profesor y Asignar Horario
      </h1>

      {/* Ocultamos el form completo mientras loading = true */}
      {!loading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <label className="flex flex-col">
                ...
                <input
                  ...
                  onChange={(e) =>
                    updateField("effectiveMonth", e.target.value)
                  }
                  required
                />
              </label>
        ...
        <div
          className=" p-10 bg-white/80 backdrop-blur-sm flex items-center justify-center rounded-2xl"
          aria-live="polite"
          aria-busy="true"
        >
          <div className="flex flex-col items-center gap-3">
            <ClipLoader size={36} />
            <p className="text-sm text-gray-700">Creando profesor…</p>
          </div>
        </div>
      )}
    </main>
