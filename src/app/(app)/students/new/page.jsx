...
export default function NewStudentAndReschedulePage() {
  // -------------- A) Crear + Inscribir --------------
  ...
  async function searchEnrollments() {
    ...
    setResResult(null);
    try {
      const url = new URL(`/api/enrollments/search`, window.location.origin);
      url.searchParams.set("email", emailSearch);
      const res = await fetch(url.toString());
      const data = await res.json();
      if (!res.ok) throw new Error(data?.error || "No se pudo buscar");
      setEnrollments(data.enrollments || []);
      if ((data.enrollments || []).length) {
        setSelEnrollmentId(data.enrollments[0]._id);
      }
    } catch (e) {
  ...
  async function submitReschedule(e) {
    ...
    setResResult(null);
    try {
      const res = await fetch("/api/students/reschedule", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ enrollmentId: selEnrollmentId, fromISO, toISO }),
      });
      const data = await res.json();
  ...
  return (
    <main className="max-w-5xl mx-auto p-6 space-y-10">
      ...
      <section className="bg-white rounded-2xl shadow p-5 space-y-4">
        <h2 className="text-lg font-medium">
          Reprogramar una clase (máx. 1 por mes)
        </h2>
        <div className="flex flex-col md:flex-row gap-3 items-end">
          <label className="flex flex-col">
            <span className="text-sm text-gray-600 mb-1">
              Buscar por email del estudiante
            </span>
            <input
              className="border rounded-lg px-3 py-2"
              value={emailSearch}
              onChange={(e) => setEmailSearch(e.target.value)}
              placeholder="alumno@dominio.com"
            />
          </label>
          <button
        ...
        {enrollments.length > 0 && (
          <div className="space-y-3">
            <label className="flex flex-col">
              <span className="text-sm text-gray-600 mb-1">Inscripción</span>
              <select
                className="border rounded-lg px-3 py-2"
                value={selEnrollmentId}
                onChange={(e) => setSelEnrollmentId(e.target.value)}
              >
                {enrollments.map((e) => (
                  <option key={e._id} value={e._id}>
                    {e.year}-{String(e.month).padStart(2, "0")} | Profe:{" "}
            ...
            {resResult?.ok && (
              <p className="text-sm text-green-700">
                Reprogramación creada (ID: {resResult.rescheduleId})
              </p>
            )}
          </div>
        )}
