...
export async function POST(req) {
  try {
    ...
    const capacity = Math.max(
      1,
      Number(profDoc?.capacidadPorFranja ?? profDoc?.capacity ?? 10)
    );
    const cupoQuery = {
    ...
    // Payload base (guardamos ambos profesores y mantenemos 'proffesor' por compatibilidad)
    const baseUpdate = {
      enrollment: en._id,
      student: en.student || en.estudiante,
      fromProfessor,
      toProfessor,
      proffesor: toProfessor, // compat con código legado
      year,
      month,
      fromDate: from,
      toDate: toFinal,
